...
def get_text_messages(message):
    ...
    if message.text == "/start":
        bot.send_message(message.from_user.id, "Подскажу названия столиц и стран. Введите более двух первых букв названия столицы или страны:")
    else:
            ...
            if len(stroka1) > 2:
                ...
                for i in range(len(spisok)):
                    ...
                    if stroka01.find(stroka1) >= 0 or stroka1.find(stroka01) >= 0 or stroka02.find(stroka1) >= 0 or stroka1.find(stroka02) >= 0:
                        ...
                        chec += 1
                        fin_stroka.append("Столица " + spisok[i][1] + spisok[i][2] + "  - город " + spisok[i][0])
                        bot.send_message(message.chat.id, '\n'.join(fin_stroka))
                if chec == 0:
                    bot.send_message(message.from_user.id, "Ничего не найдено. Попробуйте буквы другой части слова")
            else:
                bot.send_message(message.from_user.id, "Попробуйте ввести более двух букв")
        else:
